...
function getApiKey(): string {
  ...
  if (!envKey) {
    throw new Error("OpenRouter API key not found. Please set your API key in Settings or add VITE_OPENROUTER_API_KEY to .env.local");
  }
  return envKey;
}
...
/**
 * Remove OpenRouter API key from localStorage
 */
 ...
 */
async function callOpenRouter(prompt: string, model: string): Promise<AnalysisResult> {
...
/**
 * Runs a deep analysis using DeepSeek R1 (free, high-quality reasoning model)
 */
export async function runDeepAnalysis(prompt: string): Promise<AnalysisResult> {
  return callOpenRouter(prompt, 'deepseek/deepseek-r1:free');
}

/**
 * Runs a fast analysis using Gemini 2.5 Flash (free)
 */
export async function runFastAnalysis(prompt: string): Promise<AnalysisResult> {
  return callOpenRouter(prompt, 'google/gemini-2.5-flash-image-preview:free');
}

/**
 * Runs a balanced analysis using DeepSeek Chat (free)
 */
export async function runBalancedAnalysis(prompt: string): Promise<AnalysisResult> {
  return callOpenRouter(prompt, 'deepseek/deepseek-chat:free');
}
...
/**
 * Chat service for interactive conversations with OpenRouter
 */
...
/**
 * Creates a new OpenRouter chat service instance
 */
export function createChatService(): OpenRouterChatService {
